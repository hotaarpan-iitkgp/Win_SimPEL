...
void ScopeWindow::render(CircuitSimEngine::CircuitSimulator& simulator, const std::string& projectBaseName) {
    ...
    // Pull only the newly produced samples into the local cache. Copying the whole
    // telemetry every frame meant deep-copying the entire run history (all signals)
    // once per frame, per open scope window.
...
void ScopeWindow::renderToolbar(const CircuitSimEngine::TelemetryData& data, const std::string& projectBaseName) {
    ...
    if (cursorState.showCursors) {
        ...
        if (cursorState.lockDeltaPeriod) {
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.15f, 0.65f, 0.35f, 1.0f));
            char lkBuf[64];
            snprintf(lkBuf, sizeof(lkBuf), "Lock Δt (%.0fHz)##sz", cursorState.fundamentalFreq);
            if (ImGui::Button(lkBuf)) cursorState.lockDeltaPeriod = false;
            ImGui::PopStyleColor();
        } else {
            if (ImGui::Button("Lock Δt (f0)##sz")) {
...
void ScopeWindow::renderHarmonicsWindow(const CircuitSimEngine::TelemetryData& data) {
    ...
    if (ImGui::Checkbox("Lock Δt (Period)##lockDelta_scope", &cursorState.lockDeltaPeriod)) {
        if (cursorState.lockDeltaPeriod) {
            cursorState.showCursors = true;
            double deltaT = (cursorState.fundamentalFreq > 1e-6) ? ((double)cursorState.numPeriods / cursorState.fundamentalFreq) : 0.02;
            double tMin = data.timeHistory.front();
    ...
    if (ImGui::InputDouble("f0 (Hz)##f0_scope", &cursorState.fundamentalFreq, 1.0, 10.0, "%.2f")) {
    ...
    if (ImGui::InputInt("Cycles##numCyc_scope", &cursorState.numPeriods, 1, 1)) {
    ...
    ImGui::SetNextItemWidth(90.0f);
    ImGui::InputInt("Max Order (N)##maxH", &cursorState.maxHarmonics, 10, 100);
