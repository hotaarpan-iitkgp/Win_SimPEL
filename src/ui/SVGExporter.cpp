...
static std::string findBrowserExecutable() {
...
bool SVGExporter::convertHtmlToPdf(const std::string& htmlPath, const std::string& pdfPath) {
    std::string browserPath = findBrowserExecutable();
    ...
    try {
        ...
        std::string htmlUrl = "file:///" + absHtml.generic_string();
        std::string cmd = "\"" + browserPath + "\" --headless --disable-gpu --no-pdf-header-footer --print-to-pdf=\"" + absPdf.string() + "\" \"" + htmlUrl + "\"";
        ...
        if (CreateProcessA(NULL, cmdBuf.data(), NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
            ...
            return std::filesystem::exists(absPdf);
        }
    } catch (...) {}
    return false;
}
    ...
    const ReportExportOptions& options)
{
    ...
    if (options.includeSchematicSvg) {
        exportSchematicToSVGString(design, schematicSvg, false /* Light Mode Only */);
    }

    std::ofstream out(filename);
    if (!out.is_open()) return false;
    std::string jsonName = "Circuit.json";
    try {
        std::filesystem::path p(filename);
        std::string stem = p.stem().string();
        if (stem.length() > 7 && stem.substr(stem.length() - 7) == "_report") {
    ...
    std::string formatLabel = "Format: Light Mode Document HTML";
    if (options.format == ReportExportFormat::PDF) formatLabel = "Format: PDF Document";
    else if (options.format == ReportExportFormat::BOTH_HTML_PDF) formatLabel = "Format: HTML & PDF Document";

    out << "<!DOCTYPE html>\n<html lang=\"en\">\n<head>\n";
    out << "<meta charset=\"UTF-8\">\n";
    out << "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n";
    out << "<title>Circuit Simulation & Verification Report - " << jsonName << "</title>\n";
    out << "<style>\n";
    out << "  :root {\n";
    out << "    --bg-color: #f8fafc;\n";
    out << "    --card-bg: #ffffff;\n";
    ...
    out << "  @media print {\n";
    out << "    body { padding: 0; background: #fff; }\n";
    out << "    .card { box-shadow: none; border: 1px solid #cbd5e1; break-inside: avoid; page-break-inside: avoid; margin-bottom: 16px; }\n";
    out << "    .copy-btn { display: none !important; }\n";
    out << "    details { display: block !important; }\n";
    out << "    pre { max-height: none !important; overflow: visible !important; }\n";
    out << "  }\n";
    out << "</style>\n</head>\n<body>\n";
    out << "  <div class=\"header\">\n";
    out << "    <h1>Circuit Simulation & Verification Report: <span style=\"color: var(--text-main); font-weight: 500;\">" << jsonName << "</span></h1>\n";
    out << "    <div class=\"meta-bar\">\n";
    out << "      <div class=\"meta-item meta-schematic\">Schematic: " << jsonName << "</div>\n";
    out << "      <div class=\"meta-item\">Generated by SimPEL / CircuitSim Pro</div>\n";
    out << "      <div class=\"meta-item\">" << formatLabel << "</div>\n";
    out << "    </div>\n";
    out << "  </div>\n\n";

    int sectionIdx = 1;

    if (options.includeSchematicSvg && !schematicSvg.empty()) {
        out << "  <div class=\"card\">\n";
        out << "    <div class=\"card-title\">" << sectionIdx++ << ". Circuit Schematic</div>\n";
    ...
    if (options.includeSchematicJson && !schematicJson.empty()) {
        out << "  <div class=\"card\">\n";
        if (options.jsonCollapsible) {
            out << "    <details" << (options.jsonDefaultExpanded ? " open" : "") << ">\n";
            ...
            out << "    <div class=\"card-title\">" << sectionIdx++ << ". Schematic JSON Structure\n";
            out << "      <button class=\"copy-btn\" onclick=\"copyToClipboard('schematic-json-text')\">Copy JSON</button>\n";
            out << "    </div>\n";
            out << "    <pre id=\"schematic-json-text\">" << xmlEscape(schematicJson) << "</pre>\n";
        }
    ...
    if (options.includeNetlistJson && !netlistJson.empty()) {
        out << "  <div class=\"card\">\n";
        if (options.jsonCollapsible) {
            out << "    <details" << (options.jsonDefaultExpanded ? " open" : "") << ">\n";
            ...
            out << "    <div class=\"card-title\">" << sectionIdx++ << ". Netlist JSON Specification\n";
            out << "      <button class=\"copy-btn\" onclick=\"copyToClipboard('netlist-json-text')\">Copy Netlist</button>\n";
            out << "    </div>\n";
            out << "    <pre id=\"netlist-json-text\">" << xmlEscape(netlistJson) << "</pre>\n";
        }
    ...
    if (options.format == ReportExportFormat::PDF || options.format == ReportExportFormat::BOTH_HTML_PDF) {
        std::filesystem::path p(filename);
        std::string pdfPath = p.parent_path().string() + "/" + p.stem().string() + ".pdf";
        convertHtmlToPdf(filename, pdfPath);
    }
...
bool SVGExporter::exportMergedReportToHTML(
    ...
    out << "  @media print {\n";
    out << "    body { padding: 0; background: #fff; }\n";
    out << "    .toc-card, .back-top, .copy-btn { display: none !important; }\n";
    out << "    .circuit-section { page-break-before: always; break-before: page; margin-bottom: 24px; border-top: none; padding-top: 0; }\n";
    out << "    .card { box-shadow: none; border: 1px solid #cbd5e1; break-inside: avoid; page-break-inside: avoid; margin-bottom: 14px; }\n";
    out << "    details { display: block !important; }\n";
    out << "    pre { max-height: none !important; overflow: visible !important; }\n";
    out << "  }\n";
    out << "</style>\n</head>\n<body>\n";
    ...
    for (size_t i = 0; i < reports.size(); ++i) {
        ...
        if (options.includeSchematicSvg && !schematicSvg.empty()) {
            out << "    <div class=\"card\">\n";
            out << "      <div class=\"card-title\">" << secIdx++ << ". Circuit Schematic</div>\n";
            out << "      <div class=\"svg-wrapper\">\n";
            out << schematicSvg << "\n";
            out << "      </div>\n";
            out << "    </div>\n\n";
        ...
        // Schematic JSON
        if (options.includeSchematicJson && !item.schematicJson.empty()) {
            ...
            if (options.jsonCollapsible) {
                ...
                out << "      <div class=\"card-title\">" << secIdx++ << ". Schematic JSON Structure (" << xmlEscape(jName) << ")\n";
                out << "        <button class=\"copy-btn\" onclick=\"copyToClipboard('" << sJsonId << "')\">Copy JSON</button>\n";
                out << "      </div>\n";
                out << "      <pre id=\"" << sJsonId << "\">" << xmlEscape(item.schematicJson) << "</pre>\n";
            }
        ...
        // Netlist JSON
        if (options.includeNetlistJson && !item.netlistJson.empty()) {
            ...
            if (options.jsonCollapsible) {
                ...
                out << "      <div class=\"card-title\">" << secIdx++ << ". Netlist JSON Specification\n";
                out << "        <button class=\"copy-btn\" onclick=\"copyToClipboard('" << nJsonId << "')\">Copy Netlist</button>\n";
                out << "      </div>\n";
                out << "      <pre id=\"" << nJsonId << "\">" << xmlEscape(item.netlistJson) << "</pre>\n";
            }
